...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    async with httpx.AsyncClient() as client:
        response = await client.get("http://auth-service-url/verify_token", headers={"Authorization": f"Bearer {token}"})
...
def generate_license(current_user: dict = Depends(get_current_user),
...
def get_all_licenses(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
@app.get("/license/{id}")
def find_license(id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
